...
// Kiểm tra xem tên tài khoản đã tồn tại trong cơ sở dữ liệu hay chưa
    $check_query = "SELECT COUNT(*) as count FROM taikhoan WHERE user = '$name'";
    $result = $conn->query($check_query);
    $row = $result->fetch_assoc();

    if ($row['count'] > 0) {
        echo "Tên tài khoản đã tồn tại trong cơ sở dữ liệu.";
    } else {
        if ($_POST['password'] === $_POST['pass_2']) {
            $insert_query = "INSERT INTO taikhoan (user, pass, role) VALUES ('$name', '$password', 0)";
        ...
        <div class="wrapper-login">
            ...
            <form action="" method="post">
                ...
                <div class="register-link">
                    <p>Not a member <a href="index.php?act=login">Sign up now</a></p>
                </div>
            </form>
        </div>
